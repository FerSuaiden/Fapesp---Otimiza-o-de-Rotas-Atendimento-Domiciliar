...
try:
    ...
    print(f"  - Base de estabelecimentos carregada. Shape: {df_estabelecimentos.shape}")
    
    df_equipes = pd.read_csv('./CNES_DATA/tbEquipe202508.csv', sep=';', encoding='latin-1', dtype=str)
    print(f"  - Base de equipes carregada. Shape: {df_equipes.shape}")

    # --- ETAPA 2: IDENTIFICAR EQUIPES (LÓGICA ATUALIZADA) ---
    print("\n--- ETAPA 2: Identificando Tipos de Equipes (Nível Brasil) ---")
    
    # Define os códigos para cada categoria
